...
import { X, Plus, Crown, Medal, Trash2, Edit2, Check, Users, ShoppingCart, ChevronUp, ChevronDown } from 'lucide-react'
import { Button } from '@/components/ui/Button'
import { products, crownSizes, ballColors, formatPrice, CrownSize, BallColor, CartItemOptions } from '@/lib/products'
import { useCart } from '@/lib/cart-context'
import { TierProgress, getDiscountTier } from './TierProgress'
...
export function TeamBuilder({ isOpen, onClose }: TeamBuilderProps) {
  ...
  // Shared add item form component
  const AddItemForm = () => (
    <div className="bg-gold-50 rounded-xl p-4 border-2 border-gold-300">
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm font-semibold text-gold-700">New Item</span>
        <button
          onClick={() => setIsAddingNew(false)}
          className="text-sm text-charcoal-500 hover:text-charcoal-700 px-2 py-1"
        >
          Cancel
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        >
          <Crown className="w-6 h-6 mx-auto mb-2 text-gold-600" />
          <span className="block text-sm font-semibold">Crown</span>
          <span className="block text-xs text-charcoal-500 mt-1">$19.99</span>
        </button>
        <button
        ...
        >
          <Medal className="w-6 h-6 mx-auto mb-2 text-gold-600" />
          <span className="block text-sm font-semibold">Medal</span>
          <span className="block text-xs text-charcoal-500 mt-1">$9.99</span>
        </button>
      </div>
      ...
      {newItem.sku === 'CROWN-1' && (
        <div className="mb-4">
          <label className="block text-xs font-semibold text-charcoal-600 mb-2">Size</label>
      ...
      {/* Color - Grid layout for mobile */}
      <div className="mb-4">
        <label className="block text-xs font-semibold text-charcoal-600 mb-2">Color</label>
      ...
      <div className="mb-4">
        ...
        <input
          ...
          className="w-full px-4 py-3 rounded-lg border border-cream-300 text-base focus:border-gold-500 focus:outline-none focus:ring-2 focus:ring-gold-200 bg-white"
      ...
      <div className="mb-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="w-12 h-12 rounded-xl bg-white border border-cream-300 text-charcoal-700 hover:bg-cream-100 active:bg-cream-200 transition-colors flex items-center justify-center text-xl font-bold"
          >
            −
          </button>
          <input
            ...
            onChange={(e) => setNewItemQuantity(Math.min(100, Math.max(1, parseInt(e.target.value) || 1)))}
            className="w-20 text-center px-3 py-3 rounded-lg border border-cream-300 text-base font-bold focus:border-gold-500 focus:outline-none bg-white"
          />
          <button
            onClick={() => setNewItemQuantity(q => Math.min(100, q + 1))}
            className="w-12 h-12 rounded-xl bg-white border border-cream-300 text-charcoal-700 hover:bg-cream-100 active:bg-cream-200 transition-colors flex items-center justify-center text-xl font-bold"
          >
            +
          </button>
          <div className="flex-1 text-right">
            <span className="text-lg font-bold text-charcoal-900">
              {formatPrice(currentProduct.price * newItemQuantity)}
            </span>
          </div>
        </div>
      </div>

      <Button variant="primary" size="lg" className="w-full" onClick={handleAddItem}>
        <Plus className="w-5 h-5 mr-2" />
        Add {newItemQuantity > 1 ? `${newItemQuantity} Items` : 'to Order'}
      </Button>
    </div>
  )
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-0 md:inset-4 lg:inset-8 bg-cream-100 md:rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 md:px-6 py-3 md:py-4 border-b border-cream-300 bg-white flex-shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 md:w-10 md:h-10 bg-gold-100 rounded-xl flex items-center justify-center">
                  <Users className="w-5 h-5 text-gold-600" />
                </div>
                <div>
                  <h2 className="font-display text-lg md:text-xl tracking-wide text-charcoal-900">
                    TEAM BUILDER
                  </h2>
                  <p className="text-xs md:text-sm text-charcoal-500 hidden sm:block">
              ...
              <button
                onClick={onClose}
                className="p-2 text-charcoal-500 hover:text-charcoal-700 hover:bg-cream-100 rounded-xl transition-colors"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-32 md:pb-6">
                {items.length === 0 && !isAddingNew ? (
                  <div className="text-center py-8 md:py-12">
                    ...
                    <p className="text-charcoal-500 text-sm mb-6 px-4">
                      Add crowns or medals to start building your team order
                    </p>
                    <Button variant="primary" size="lg" onClick={() => setIsAddingNew(true)}>
                      <Plus className="w-5 h-5 mr-2" />
                      Add First Item
                    </Button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {items.map((item, index) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`bg-white rounded-xl p-3 md:p-4 shadow-sm transition-all ${
                        ...
                        >
                          {isEditing ? (
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center justify-between">
                                ...
                                <button
                                  ...
                                  className="text-sm text-gold-600 hover:text-gold-700 font-semibold px-2 py-1"
                                >
                                  Done
                                </button>
                              </div>
                              ...
                              <input
                                ...
                                className="w-full px-3 py-2.5 rounded-lg border border-cream-300 text-sm focus:border-gold-500 focus:outline-none"
                            ...
                            // Display mode
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-12 relative rounded-lg overflow-hidden flex-shrink-0 bg-cream-100">
                                <Image
                                  src={product.image}
                                  alt={product.name}
                                  fill
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  <span className="text-xs font-medium text-charcoal-400">#{index + 1}</span>
                              ...
                              <span className="text-sm font-bold text-charcoal-700 ml-1">
                    ...
                    {isAddingNew ? (
                      ...
                      <button
                        ...
                        className="w-full p-4 border-2 border-dashed border-cream-300 rounded-xl text-charcoal-500 hover:border-gold-400 hover:text-gold-600 active:bg-gold-50 transition-colors flex items-center justify-center gap-2"
                      >
              ...
              <div className="hidden md:flex w-80 bg-white border-l border-cream-300 p-6 flex-col">
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-cream-300 shadow-lg z-10">
              {/* Expandable Summary */}
              <AnimatePresence>
                {showMobileSummary && items.length > 0 && (
                  <motion.div
                    ...
                    className="overflow-hidden border-b border-cream-200"
                  >
                    <div className="p-4 bg-cream-50 max-h-48 overflow-y-auto">
                      <TierProgress
                        quantity={totalItems}
                        unitPrice={averagePrice}
                        className="mb-3"
                        ...
                        {items.slice(0, 5).map((item, i) => {
                          ...
                          return (
                            <div key={item.id} className="flex justify-between">
                              <span className="text-charcoal-600 truncate max-w-[200px]">
                                #{i + 1} {product.name.split(' ')[1]}
                              </span>
                              <span className="text-charcoal-700">{formatPrice(product.price)}</span>
                            </div>
                          )
                        })}
                        {items.length > 5 && (
                          <p className="text-charcoal-500 text-xs">+{items.length - 5} more items...</p>
                        )}
                      </div>
                    </div>
                  </motion.div>
                ...
                {items.length > 0 ? (
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-between bg-cream-100 rounded-xl px-4 py-3"
                    >
                      <div className="text-left">
                        <p className="text-xs text-charcoal-500">{totalItems} items</p>
                        <p className="font-bold text-charcoal-900">{formatPrice(discountedTotal)}</p>
                        {tier.discount > 0 && (
                          <p className="text-xs text-pickle-600 font-medium">Save {formatPrice(savings)}</p>
                        )}
                      </div>
                      {showMobileSummary ? (
                  ...
                  <div className="text-center py-2">
                    <p className="text-sm text-charcoal-500">Add items to see your order total</p>
                    {!isAddingNew && (
                      <button
                        onClick={() => setIsAddingNew(true)}
                        className="text-gold-600 font-semibold text-sm mt-1"
                      >
                        + Add First Item
                      </button>
                    )}
